...
def wishMe():
    ...
    if(hour>=0) and (hour<=12) and ('AM' in t):
        speak(f"Good morning UBU, it's {day} and the time is {t}")
    elif(hour>=12)  and (hour<=16) and ('PM' in t):
        speak(f"Good afternoon UBU, it's {day} and the time is {t}")
    else:
        speak(f"Good evening UBU, it's {day} and the time is {t}")
